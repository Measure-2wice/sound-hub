...
describe("disposable test database guard", () => {
  ...
  test("rejects 0.0.0.0 even though it was previously accepted", () => {
    assert.throws(
      () => assertDisposableTestDatabase("postgresql://soundhub:password@0.0.0.0:5433/soundhub_m1_test"),
  ...
  test("rejects a non-approved port (5432)", () => {
    assert.throws(
      () => assertDisposableTestDatabase("postgresql://soundhub:password@localhost:5432/soundhub_m1_test"),
      (err: unknown) =>
  ...
  test("rejects a non-approved database name even when it ends in _test", () => {
    assert.throws(
      () => assertDisposableTestDatabase("postgresql://soundhub:password@localhost:5433/some_other_test"),
      (err: unknown) =>
  ...
  test("rejects a missing URL", () => {
    ...
    try {
      assert.throws(() => readTestDatabaseUrl(), (err: unknown) => err instanceof TestDatabaseGuardError);
    } finally {
