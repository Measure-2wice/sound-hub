...
// B5: bounded P2034 retry mirrors `fundDealInTransaction`. The
// adapter's failure recorder runs at Serializable isolation and
// MUST use the bounded retry mechanism rather than letting a
// serialization conflict leak to the route layer. These tests
// pin the contract directly: the method retries transient
// serialization conflicts up to the budget, exhausts cleanly
// after the budget without leaking a raw Prisma P2034, and
// never demotes a Confirmed intent across retries.
test("B5: bounded P2034 retry — transient serialization conflicts are retried within the budget and then succeed", async () => {
  // Wrap the underlying Prisma $transaction to inject a single
  // synthetic P2034 on the first attempt of the failure recorder.
  // This proves the retry helper is wired into the failure path
  // (not just the Phase-3 path). The seeded intent is in the
  // Created state so the guarded UPDATE matches 1 row.
  await prisma.paymentIntent.deleteMany({ where: { dealId: DEAL_ID } });
  const created = await repo.findOrCreatePaymentIntentInTransaction({
    dealId: DEAL_ID,
    termsVersionId: TV_ID,
    ...
    actingWorkspaceId: BUYER_WORKSPACE_ID,
    createdByUserId: BUYER_USER_ID,
    correlationId: "corr_failure_retry",
  });
  if (!created.ok) throw new Error("seed failed");
  ...
  (prisma as unknown as { $transaction: typeof prisma.$transaction }).$transaction = ((
    ...
    if (!injected) {
      injected = true;
      // Use the canonical PrismaClientKnownRequestError so
      // `isSerializationConflict` recognizes the synthetic
      // conflict and the retry helper actually retries. A
      // bare `Error` with `code: "P2034"` would NOT match
      // the retry classifier — the helper requires either the
      // Prisma error type or a message that matches the
      // documented regex, neither of which a generic Error
      // satisfies in isolation.
      return Promise.reject(
        new PrismaClientKnownRequestError("could not serialize access due to concurrent update", {
          code: "P2034",
          clientVersion: "test-mock",
        }),
      ) as unknown as ReturnType<typeof prisma.$transaction>;
    }
    return originalTransaction(
      callback as Parameters<typeof prisma.$transaction>[0],
      ...(args.slice(1) as []),
    );
  ...
  try {
    ...
    assert.deepEqual(
      ...
      "a transient P2034 on the first attempt MUST be retried within the bounded budget and the second attempt must persist the failure",
    );
    const reread = await repo.findCurrentPaymentIntent(DEAL_ID);
    assert.ok(reread);
    assert.equal(reread.providerState, "Failed");
...
test("B5: bounded P2034 retry — exhaustion surfaces CONCURRENCY_RETRY_EXHAUSTED with no Confirmed→Failed demotion", async () => {
  // Same setup as the transient case, but every attempt rejects
  // with P2034. The bounded budget exhausts; the adapter returns
  // the documented exhaustion envelope rather than leaking the
  // raw Prisma conflict code. The intent row stays in its
  // pre-call state — no mutation, no demotion.
  await prisma.paymentIntent.deleteMany({ where: { dealId: DEAL_ID } });
  const created = await repo.findOrCreatePaymentIntentInTransaction({
    dealId: DEAL_ID,
    termsVersionId: TV_ID,
    ...
    actingWorkspaceId: BUYER_WORKSPACE_ID,
    createdByUserId: BUYER_USER_ID,
    correlationId: "corr_failure_retry_exhaust",
  });
  if (!created.ok) throw new Error("seed failed");
  ...
  try {
    ...
    assert.deepEqual(
      result,
      { ok: false, reason: "CONCURRENCY_RETRY_EXHAUSTED" },
      "exhausted P2034 retry MUST surface CONCURRENCY_RETRY_EXHAUSTED rather than leaking the raw Prisma conflict code",
    );
    // The intent row is untouched — Created state preserved.
    const reread = await repo.findCurrentPaymentIntent(DEAL_ID);
    assert.ok(reread);
    assert.equal(
      reread.providerState,
      "Created",
      "exhausted retry MUST NOT mutate the intent row (no Confirmed→Failed demotion, no Failed write)",
    );
    assert.equal(reread.failureReasonCode, null);
    assert.equal(reread.failureDetailCategory, null);
  } finally {
