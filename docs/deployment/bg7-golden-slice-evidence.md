...
| ------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Seller consent** (explicit ProjectRequest acceptance) | Asserted on the **seller-inbox** surface, not on the Deal page. `golden-slice.spec.ts` Step 6 clicks `data-testid="seller-request-accept"` on `/seller-requests` and asserts the success card `seller-requests-success` contains `Accepted`. The consent is persisted as `ProjectRequest.sellerConsentAt` (see `packages/types/src/index.ts:1391`); the Deal DTO does not re-render it. |
